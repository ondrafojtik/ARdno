...
namespace UnityVolumeRendering
{
    ...
    public class TransferFunction : ScriptableObject
    {
        ...
        public Texture2D GetTextureFromTo(float from, float to)
        {
            if (texture == null)
                GenerateTextureFromTo(from, to);

            return texture;

        }

        public void GenerateTextureFromTo(float from, float to)
        {
            if (texture == null)
                CreateTexture();
            ...
            // remove points that are not in scope
            List<TFColourControlPoint> filtered = new List<TFColourControlPoint>();
            for (int i = 0; i < cols.Count; i++)
            {
                if (cols[i].dataValue >= from && cols[i].dataValue <= to)
                    filtered.Add(cols[i]);
            }
            cols = filtered;
            List<TFAlphaControlPoint> filtered_ = new List<TFAlphaControlPoint>();
            for (int i = 0; i < alphas.Count; i++)
            {
                if (alphas[i].dataValue >= from && alphas[i].dataValue <= to)
                    filtered_.Add(alphas[i]);
            }
            alphas = filtered_;
